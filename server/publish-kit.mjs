...
export function buildCredits(content, scenes) {
  ...
  if (photos.size) {
    // Generated backdrops carry an ElevenLabs credit, and calling those "stock
    // photography" would be a false claim in a file whose whole job is honest
    // attribution - platforms increasingly want synthetic imagery declared.
    const drawn = [...photos].filter((c) => /generated with/i.test(c));
    const shot = [...photos].filter((c) => !/generated with/i.test(c));

    lines.push('IMAGES');
    if (shot.length) {
      lines.push('Stock photography used in this video:');
      for (const credit of shot) lines.push('  ' + credit);
    }
    if (drawn.length) {
      if (shot.length) lines.push('');
      lines.push('Some backdrops in this video were generated by AI:');
      for (const credit of drawn) lines.push('  ' + credit);
    }
    lines.push('');
  }
