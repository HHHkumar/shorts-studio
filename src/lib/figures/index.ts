...
import { BARS_FAMILY, type Bars } from './bars.ts';
import type { FigureAnswer, FigureFamily } from './family.ts';
import { formatQuantity, parseQuantity, sameValue } from './quantity.ts';

export type Figure = Circuit | Junction | AcFigure | PowerTriangle | ThreePhase | Transformer | Machine | Graph | Bars;

export const FAMILIES: FigureFamily<any>[] = [JUNCTION_FAMILY, CIRCUIT_FAMILY, AC_FAMILY, POWER_TRIANGLE_FAMILY, THREE_PHASE_FAMILY, TRANSFORMER_FAMILY, MACHINE_FAMILY, GRAPH_FAMILY, BARS_FAMILY];
...
export function figurePromptLines(subject: string, topic: string): string[] {
  ...
  const lines = [
    ...
    'Three rules decide whether a figure is worth drawing at all:',
    '1. It draws the situation THE QUESTION states - the same components, the same values, nothing',
    '   else. A picture of a different case in the same chapter is worse than no picture.',
    '2. Every value on it comes from the question. Never invent a phase angle, a voltage, a rating or',
    '   a second waveform the question does not give, however ordinary the invented value looks.',
    '3. It must MARK WHAT THE QUESTION ASKS, with the "ask" field of that type, so the drawing has an',
    '   unknown to solve. A figure that asks nothing works nothing out, cannot be checked against your',
    '   answer, and is DROPPED.',
    'So a conceptual question - one with no specific setup or values, such as how many times a',
    'waveform crosses zero, or what a term means - takes "figure": "none". Say it in the narration',
    'and let the scene carry the words.',
